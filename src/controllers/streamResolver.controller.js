...
import { isAllowedStreamUrl } from "../configs/streamProxy.config.js";
...
const getResolvedStream = async (req, res, next) => {
  try {
    ...
    const result = {
      ...
      ...resolved,
      // NOTE: resolved.skipData (per-episode, from the source CDN's own API)
      // is more accurate than streamInfo.skipData, which is frequently just
      // a [0,0] placeholder at this earlier stage — prefer it when present.
      skipData: resolved.skipData || streamInfo.skipData,
    };

    // NOTE: Only cache successful resolutions — resolveStreamUrl never
    // throws on failure (it returns {url: null, error}), so caching
    // unconditionally would turn a one-off upstream hiccup into a
    // guaranteed failure for every request within the TTL window.
    if (result.url) {
...
const getStreamQualities = async (req, res, next) => {
  try {
    ...
    if (!isAllowedStreamUrl(url)) {
      return res.status(403).json({ success: false, message: "Domain not allowed for this operation" });
    }
