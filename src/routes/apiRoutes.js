...
import { isAllowedStreamUrl, isSafeExternalUrl, looksLikeVideoSegment, resolveRefererOrigin } from "../configs/streamProxy.config.js";
import { classifySegments } from "../helper/streamSegmentGuard.helper.js";
...
app.use((req, res, next) => {
  ...
  app.get("/api/stream/proxy", async (req, res, next) => {
    try {
      ...
      // NOTE: Domains in ALLOWED_STREAM_DOMAINS are trusted immediately.
      // Anything else must still pass the baseline SSRF safety gate
      // (scheme + not a private/internal address) — segment CDNs rotate
      // per provider/server, so a static allowlist alone keeps missing
      // legitimate domains (see streamSegmentGuard.helper.js); the actual
      // "is this really a playlist" check happens after the fetch below.
      const urlAllowlisted = isAllowedStreamUrl(url);
      if (!urlAllowlisted && !(await isSafeExternalUrl(url))) {
      ...
      const refererOrigin = resolveRefererOrigin(ref);
      ...
      // NOTE: Rewrite relative URLs in M3U8 to proxy URLs, preserving ref
      // so nested segment/playlist requests keep using the correct Referer
      let content = typeof response.data === "string" ? response.data : String(response.data);

      // NOTE: for domains outside the curated allowlist, verify the
      // response is actually a playlist before proceeding — the entry
      // point accepts any SSRF-safe URL now, so this content check is
      // what keeps it from being usable as a generic text-fetch relay.
      if (!urlAllowlisted && !content.trimStart().startsWith("#EXTM3U")) {
        return res.status(403).json({ success: false, message: "Response is not a valid M3U8 playlist" });
      }

      const baseUrl = url.substring(0, url.lastIndexOf("/") + 1);
      const refParam = ref ? `&ref=${encodeURIComponent(ref)}` : "";

      // NOTE: Some providers interleave ad segments directly into the
      // timeline, not just as pre/post-roll. Rewriting them to a proxy
      // URL that will 403 leaves an unplayable gap wherever a fragment
      // lands on one — which is exactly what breaks seeking. Instead,
      // drop each ad segment's URI line *and* its preceding #EXTINF:
      // line entirely, so the manifest hls.js sees only ever contains
      // segments we can actually serve.
      //
      // Distinguishing "ad" from "real" isn't just a domain allowlist
      // check anymore — verified live, real episode segments are
      // sometimes served from rotating CDN domains outside our curated
      // list, disguised with fake image content-types/signatures (the
      // same trick already known from g4stw.livedns.my). classifySegments
      // does the real work: allowlisted domains are trusted instantly;
      // anything else is content-probed (real MPEG-TS/fMP4 signature vs.
      // an actual ad creative) before being trusted. See
      // streamSegmentGuard.helper.js.
      //
      // The surviving real segments are NOT contiguous in the original
      // timeline (they're sparse, with runs of dropped ad segments in
      // between) — splicing them back-to-back without saying so makes
      // the decoder treat a big PTS jump as continuous footage, which
      // manifests as the clock/audio advancing while the video frame
      // freezes on the last successfully decoded picture. #EXT-X-
      // DISCONTINUITY right before the next kept segment after a drop
      // tells the player to reset its timeline expectations there
      // instead of assuming continuity.
      const lines = content.split(/\r?\n/);

      // First pass: parse structure only. Segments on domains outside
      // ALLOWED_STREAM_DOMAINS need an async classification (SSRF-safety
      // + content probe) before we know whether to keep or drop them —
      // collect those separately so they can be probed in parallel
      // rather than one-by-one down the line.
      const parsed = [];
      for (const line of lines) {
        const trimmed = line.trim();
        if (!trimmed) { parsed.push({ type: "blank", line }); continue; }
        if (trimmed.startsWith("#")) {
          parsed.push(/^#EXTINF:/i.test(trimmed) ? { type: "extinf", line } : { type: "tag", line });
          continue;
        }
        // Per the HLS spec, every non-comment, non-blank line is a URI
        // (nested playlist or media segment) — segment URIs aren't
        // guaranteed to end in ".ts", so route on whether the resolved
        // URL references another .m3u8 playlist instead of extension.
        const absoluteUrl = trimmed.startsWith("http") ? trimmed : new URL(trimmed, baseUrl).href;
        parsed.push({ type: "uri", absoluteUrl, isNestedPlaylist: absoluteUrl.includes(".m3u8") });
      }

      const toClassify = parsed
        .filter((p) => p.type === "uri" && !isAllowedStreamUrl(p.absoluteUrl))
        .map((p) => ({ url: p.absoluteUrl, isNestedPlaylist: p.isNestedPlaylist, refererOrigin }));
      const classified = toClassify.length ? await classifySegments(toClassify) : null;

      const outputLines = [];
      let pendingExtinf = null;
      let needsDiscontinuity = false;
      let hasOutputSegment = false;
      for (const item of parsed) {
        if (item.type !== "uri") {
          if (item.type === "extinf") { pendingExtinf = item.line; }
          else { outputLines.push(item.line); }
          continue;
        }
        const keep = isAllowedStreamUrl(item.absoluteUrl) || classified?.get(item.absoluteUrl) === true;
        if (!keep) {
          pendingExtinf = null;
          needsDiscontinuity = hasOutputSegment;
          continue;
        }
        ...
        if (pendingExtinf) outputLines.push(pendingExtinf);
        pendingExtinf = null;
        const endpoint = item.isNestedPlaylist ? "/api/stream/proxy" : "/api/stream/ts-proxy";
        outputLines.push(`${endpoint}?url=${encodeURIComponent(item.absoluteUrl)}${refParam}`);
  ...
  app.get("/api/stream/ts-proxy", async (req, res, next) => {
    try {
      ...
      // NOTE: same relaxed-but-verified model as /api/stream/proxy above —
      // allowlisted domains skip straight through; anything else must
      // pass the SSRF safety gate now and the content-signature check
      // after fetching (see streamProxy.config.js / streamSegmentGuard).
      const urlAllowlisted = isAllowedStreamUrl(url);
      if (!urlAllowlisted && !(await isSafeExternalUrl(url))) {
        return res.status(403).json({ success: false, message: "Domain not allowed for proxy" });
      }
      ...
      if (!urlAllowlisted && !looksLikeVideoSegment(buffer)) {
        return res.status(403).json({ success: false, message: "Response is not a recognized video segment" });
      }
