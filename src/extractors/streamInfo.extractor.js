...
const extractStreamInfo = async (linkId, watchSlug = null) => {
  try {
    ...
    if (watchSlug) {
      await fetchWithMirror(`/watch/${encodeURIComponent(watchSlug)}`);
    }

    const path = `/ajax/server?get=${encodeURIComponent(linkId)}`;
...
const extractServerList = async (episodeIds, watchSlug = null) => {
  try {
    ...
    const path = `/ajax/server/list?servers=${encodeURIComponent(episodeIds)}`;
    const { data: raw } = await fetchWithMirror(path, {
      headers: { "X-Requested-With": "XMLHttpRequest" }
    });

    // NOTE: raw is the JSON-encoded {status, result} envelope as text
    // (fetchWithMirror's default returnType), not bare HTML — parse it
    // first, otherwise cheerio loads the JSON string itself and finds no matches.
