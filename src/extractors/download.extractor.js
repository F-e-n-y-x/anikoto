...
const extractDownloadLinks = async (slug, ep) => {
  try {
    const { data } = await fetchWithMirror(`/watch/${encodeURIComponent(slug)}/ep-${encodeURIComponent(ep)}`);
            ...
            for (const [serverGroup, servers] of Object.entries(decoded)) {
              for (const [serverName, url] of Object.entries(servers)) {
                ...
                if (!exists) {
                  ...
                  // match any subdomain (not just "pahe."), replacing only the
                  // origin so the rest of the path is preserved.
