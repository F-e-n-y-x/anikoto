...
// Embed domain to API domain overrides — empty by default since the
// source API lives on the embed's own domain (verified against
// megaplay.buzz: /stream/getSourcesNew is served from megaplay.buzz
// itself, not a separate "-1" domain as previously assumed here).
const EMBED_API_MAP = {};
...
const resolveStreamUrl = async (embedUrl, options = {}) => {
  const { followRedirects = true, timeout = 10000, retries = 1 } = options;
  ...
  try {
    ...
    // NOTE: Query the megaplay-style API for the actual stream URL
    const apiResponse = await axios.get(
      `https://${apiDomain}/stream/getSourcesNew?id=${encodeURIComponent(dataId)}`,
      {
        headers: {
          ...headers,
          "Referer": embedUrl,
          "Origin": `https://${embedDomain}`,
          // NOTE: this endpoint intermittently 403s with "This endpoint
          // accepts only AJAX requests" when this header is absent —
          // verified live. Harmless to always send it.
    ...
    // NOTE: Extract stream URL from API response — the real shape nests
    // it under sources.file (confirmed live); other keys kept as fallback
    // in case a different embed provider's response shape differs.
    const streamUrl = apiData?.sources?.file || apiData?.source || apiData?.url || apiData?.file || null;
    ...
    return {
      ...
      qualities,
      subtitles,
      skipData: apiData?.skip_data || (apiData?.intro || apiData?.outro ? { intro: apiData.intro || null, outro: apiData.outro || null } : null),
      dataId,
      realId,
      mediaId,
    ...
    // NOTE: this upstream is known to intermittently 403 (Cloudflare
    // edge-dependent AJAX-header enforcement) — a short retry recovers
    // most of those without giving up on a real, working stream.
    if (attempt < retries) {
...
const extractSubtitles = (apiData) => {
  ...
  if (apiData?.tracks && Array.isArray(apiData.tracks)) {
    for (const track of apiData.tracks) {
      if (track.kind === "subtitles" || track.kind === "captions" || track.type === "subtitles") {
        subtitles.push({
