...
const extractLatestUpdated = async (page = 1) => {
  try {
    ...
    const totalPages = countPages($);
    // NOTE: no selector override — let parseListItems try "#list-items >
    // .item" first so it doesn't also pick up the page's unrelated static
    // sidebar widget (see parseListItem.helper.js for details).
    const results = parseListItems($);
